makeForecast <- function(state_abbr = "AZ", 
  ...
  # might need to put in some protections around negative smoothed values....
  # might need to add in late G?
  noise <- diag(G)
  set.seed(r.seed)

  nG <- G + MASS::mvrnorm(n = 1, mu = rep(0,length(G)),
                            Sigma = noise)
...
plt <- ggplot(out_data |>
                filter(date <= as.Date(forecast_date, format = "%m-%d-%Y") + forecast_window + 90)) +
  geom_point(mapping = aes(x = date, y = positiveIncrease)) +
  geom_line(mapping = aes(x = date, y = smoothedIncidence, color = "smoothed data"), linewidth = 1,
            col = "black")+
  geom_point(out_data |>
               filter(status %in% c("available","historical")), mapping = aes(x = date, y = smoothedIncidence, color = status))+
  geom_line(mapping = aes(x = date, y = predictedIncidence, color = "prediction"), linewidth = 1)

return(list(plt = plt, dat = out_data, forecast_date = as.Date(forecast_date, format = "%m-%d-%Y")))
}


predictSIRICC <- function(initC = 100, times = seq(0, 30, 1), 
